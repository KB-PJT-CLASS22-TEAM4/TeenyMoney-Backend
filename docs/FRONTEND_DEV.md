...
백엔드 API 계약의 기준은 springfox가 Controller와 DTO에서 생성한 Swagger 문서입니다.
...
현재 문서는 Java 코드의 Swagger 애노테이션에서 자동 생성됩니다. 애노테이션이 없으면
설명과 예시가 비어 있는 채로 노출됩니다.

> 백엔드 개발자는 도메인 API를 추가하거나 DTO와 응답 계약을 변경할 때
> Controller의 `@Api`, `@ApiOperation`, `@ApiResponses`와 DTO의 `@ApiModel`,
> `@ApiModelProperty`를 같은 작업에서 반드시 갱신해야 합니다.
> Swagger UI에서 확인하기 전에는 프론트엔드에 API 구현 완료를 알리지 않습니다.
...
```text
https://www.teenymoney.kro.kr/v2/api-docs
```
...
Collection이 아니라 `/v2/api-docs`가 노출하는 Swagger 문서입니다.
...
3. Controller와 DTO에 Swagger 애노테이션 갱신
...
API 계약이 코드와 다르면 구두 설명만으로 맞추지 말고 Swagger 애노테이션을 먼저 수정합니다.
...
구현 전 화면 개발이 필요하면 Swagger에 확정된 요청·응답 예시를 먼저 정의하고 Postman
Mock Server 또는 MSW를 사용할 수 있습니다. Mock 응답 역시 Swagger 계약과 일치해야
합니다.
...
https://www.teenymoney.kro.kr/swagger-ui.html
https://www.teenymoney.kro.kr/v2/api-docs
```
