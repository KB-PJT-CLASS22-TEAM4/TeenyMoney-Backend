...
별도 배치 서버는 없습니다. `@EnableScheduling`이 `RootConfig`에 있어서 **뜨는 모든
인스턴스가 아래 작업을 각자 돌립니다.** 인스턴스를 늘리면 실행 횟수도 그만큼 늘어납니다.

| 작업 | 주기 | 조정 방법 |
| --- | --- | --- |
| 퀘스트 마감 (`QuestDeadlineScheduler`) | `fixedDelay` 60초 | 코드 상수 |
| 금융상품 동기화 (`FinancialProductSyncScheduler`) | `fixedDelay` 24시간, 기동 60초 후 첫 실행 | `finlife.sync.fixed-delay-ms`, `finlife.sync.initial-delay-ms` |
| 티니점수 월간 등급 (`TeenyScoreGradeScheduler`) | 매월 1일 00:00 KST | 코드 상수 |

#### 퀘스트 마감 배치

기한이 지난 `AVAILABLE`을 `EXPIRED`로, `IN_PROGRESS`를 `FAILED`로 바꿉니다.
`PENDING`은 부모가 검토할 수 있어야 하므로 대상이 아닙니다.

동작 한계값은 전부 `QuestDeadlineService`의 상수입니다. 환경변수나 프로퍼티가 아니므로
바꾸려면 코드를 고쳐 재배포해야 합니다.

| 상수 | 값 | 의미 |
| --- | --- | --- |
| `LOCK_BATCH_SIZE` | 200 | 한 번에 잠그는 행 수. 각 묶음이 별도 트랜잭션 |
| `MAX_PER_RUN` | 2,000 | 한 주기 처리 상한. 남으면 다음 주기가 이어서 처리 |
| `MAX_RUN_DURATION` | 20초 | 한 주기 시간 상한 |

**여러 대를 띄워도 안전합니다.** 대상 조회가 `FOR UPDATE SKIP LOCKED`라 다른 인스턴스가
잠근 행은 기다리지 않고 건너뛰고, 상태 변경은 `AND status = 이전상태` 조건을 달아
같은 퀘스트의 상태와 티니점수가 두 번 반영되지 않습니다.

운영 시 확인할 것:

- **NTP 동기화가 필수입니다.** 마감 판정이 서버 시각 기준이라 시계가 밀리면 아직 기한이
  남은 퀘스트를 종료하거나, 지난 퀘스트를 방치합니다.
- 처리량 상한은 인스턴스 한 대 기준 **분당 2,000건**입니다. 밀린 물량이 이보다 크면
  여러 주기에 걸쳐 소화됩니다. 초기 목표 규모(전체 10만 건 이하)에서는 여유가 있습니다.
- 정상 동작은 애플리케이션 로그의 `퀘스트 마감 처리 N건`으로 확인합니다. 마감할 것이
  없으면 아무것도 남기지 않으므로, 로그가 조용한 것 자체는 이상이 아닙니다.
- 실패는 `퀘스트 마감 배치 실패`로 남고 **예외를 삼킵니다.** 다음 주기에 다시 시도하며,
  이 로그가 계속 쌓이면 DB 연결이나 잠금 경합을 봅니다.
- 특정 인스턴스에서만 배치를 끄는 스위치는 없습니다. 필요해지면 프로파일이나 프로퍼티
  조건을 `QuestDeadlineScheduler`에 추가해야 합니다.
- 트랜잭션 매니저의 `nestedTransactionAllowed`가 켜져 있어야 합니다(`RootConfig`에 명시).
  SAVEPOINT를 지원하지 않는 매니저로 바꾸면 마감이 매 주기 실패합니다.
...
| 기한이 지났는데 퀘스트가 안 닫힘 | 배치는 1분 주기다. 그 뒤에도 남으면 서버 시각(NTP)과 `퀘스트 마감 배치 실패` 로그 확인 |
| 밀린 마감이 한 번에 안 끝남 | 정상. 한 주기 상한이 2,000건이라 나눠서 처리된다 |
| 인증 사진이 90일 뒤에도 남음 | 버킷에 Lifecycle 규칙 미적용. `get-bucket-lifecycle-configuration`으로 확인 |
